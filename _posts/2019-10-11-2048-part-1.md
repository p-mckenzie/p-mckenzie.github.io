...
![]({{site.url}}/assets/img/2048/2048.gif)
...
```
So we're storing: 
* `layouts` in a `16xn` matrix, where each row is a layout during the game
* `moves` in a `4xn` matrix, where each row is one-hot encoded the `w`, `s`, `a`, `d` choice
* `scores` in a flat array of length `n`

<br>
Note we've performed a single move (down-swipe) so far in our demonstration game, so `n=1`. 
...
Our current `game.scores` looks like:<br>`array([0])`

### Playing the game
We have all the functionality we need! Out of personal pride I decided to also cobble together a relatively
decent user interface, which takes that play numpy array (`game.layout`) and at least puts colors on it.

Far from beautiful, but at least reminiscent of the original design:
...
This implementation is available in the GitHub repository, and can be played (basic UI and all), out of 
