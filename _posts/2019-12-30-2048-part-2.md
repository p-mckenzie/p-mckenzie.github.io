...
is to the "worst", and each "good" game is to the "best". Closer to the median means closer to $$j=0$$, while closer to the 
...
* `linear_move_num` - how close the move is to the end of the game, $$\frac{\text{Move number}}{\text{total number of moves in game/mini-game}}$$
